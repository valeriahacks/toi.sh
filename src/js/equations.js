...
/*11*/	(j, i, x) => Math.floor((0, _sinh2.default)((i ^ j * (x * 2) * Math.sin(x)) / (Math.sin(x * j) * 255)) * 255),
/*12*/	(j, i, x) => Math.floor(Math.tan(Math.sin(x * j) / Math.cos(x * i)) * 255),
/*13*/	(j, i, x) => Math.floor(Math.sin(x * 1 / j * i / x - i ^ x) * 255),
/*14*/	(j, i, x) => Math.floor(Math.tan(x - i - j) / (0, _tanh2.default)((j - (x * 5000 * x ^ x ^ x ^ x ^ i ^ j ^ x ^ x)) / (i * x) / i + j) * 255),
